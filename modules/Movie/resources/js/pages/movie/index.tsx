    ...
    return (
        <div className="min-h-screen bg-[#030303] text-white font-sans relative overflow-x-hidden">
            <ThreeBackground />
                ...
                <nav className="fixed w-full z-50 flex items-center justify-between px-8 py-6 bg-black/50 backdrop-blur-xl border-b border-white/5">
                    <div className="text-xl font-bold tracking-tighter text-white uppercase tracking-widest">
                        MovieFinder
                    </div>
                    <div className="flex items-center gap-8 font-medium text-sm text-gray-400">
                        ...
                        <form action="/logout" method="POST">
                            ...
                            <button type="submit" className="hover:text-white transition-colors uppercase tracking-widest text-xs font-bold">Logout</button>
                        </form>
                    </div>
                </nav>
            ...
            <main className="pt-24 px-8 max-w-7xl mx-auto pb-20 relative z-10">
                ...
                >
                    ...
                    <div className="relative w-full md:w-96 gap-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6">
                    {movies.map((m, i) => (
                        <motion.a
                            ...
                            className="group relative block aspect-[2/3] rounded-lg overflow-hidden bg-gray-900 border border-white/5 transition-all duration-300 hover:border-white/20 hover:shadow-2xl"
                        >
                            <LazyImage src={m.Poster} alt={m.Title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
                            <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-4">
                                <h3 className="font-bold text-white text-sm line-clamp-2">{m.Title}</h3>
                                <div className="flex items-center gap-2 mt-2 text-gray-400 text-xs">
                                    <span>{m.Year}</span>
                                    <span className="capitalize px-1.5 py-0.5 rounded bg-white/10 text-[10px]">{m.Type}</span>
                                </div>
                            </div>
                        </motion.a>
                    ))}
